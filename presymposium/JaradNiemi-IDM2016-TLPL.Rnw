...
\frame{\frametitle{Conditional forward propagation}
	Recall the model
	\[ \begin{array}{rll}
	Y_{t} & \sim Bin(\Delta X_{t},\theta_{}) \\
	\Delta X_{t} & \sim Po(\lambda f(X_{t-1}))
	\end{array} \]
	
	\vspace{0.2in} \pause 
	
	Then $p(\Delta X_t|\lambda, \theta,X_t, y_{t})$ is 
	\[ \begin{array}{rl}
	\Delta X_{t} &= y_{t} + Z_{t} \\
	Z_{t} & \sim Po([1-\theta]\lambda f(X_{t-1}))
	\end{array} \]
	\pause 
	by an appeal to Bayes' Rule, a change of variables, and the marginal distribution for $Y_t$:
	\[ Y_{t}  \sim Po(\theta\lambda f(X_{t-1})).\]
