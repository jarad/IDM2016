...
dd = ddply(expand.grid(p_SI = seq(0,1,by=1/2),
                      p_IR = seq(0,1,by=1/2)),
          .(p_SI,p_IR), 
          function(x,df) {
...
<<noisy_observations_data, dependson=c('noisy_observations','colors')>>=
...
<<noisy_data_forecasts,dependson='noisy_observations'>>=
sir = sckm('sir')
tp = 20
...
<<dependson=c('noisy_data_forecasts','colors')>>=
pred = pred %>%
